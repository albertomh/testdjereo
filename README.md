...
subclass it will have a UUID as a primary key instead of the default `BigAutoField`.
This is used by the models in the existing 'users' app and can be used by any new models.
...
The `_deploy/` directory of includes a Dockerfile that uses the local development toolchain
...
The container's entrypoint runs [gunicorn](https://docs.gunicorn.org/en/latest/index.html)
as the WSGI web server to make the Django application available.

Build an deployment-ready image with:
...
The recommended use of the resulting container is to use [nginx](https://nginx.org/en/docs/index.html)
as the reverse proxy, with `gunicorn` as the WSGI web server.
